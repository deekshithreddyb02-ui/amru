...
import { Loader2, MapPin, CheckCircle2, Clock } from "lucide-react";
...
const Admin = () => {
  ...
  const { isAdmin, isSuperAdmin, loading: roleLoading, mustChangePassword } = useUserRole();
  ...
  useEffect(() => {
    ...
    if (isSuperAdmin) {
      // Super admins use the full dashboard
      navigate("/super-admin");
    }
  }, [isAdmin, isSuperAdmin, roleLoading, mustChangePassword, navigate]);
  ...
  const fetchAssignedLeads = async () => {
    ...
    try {
      ...
      toast({ title: "Failed to load leads", description: error?.message || "Please try again", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Card key={lead.id} className={lead.is_completed ? "opacity-80" : ""}>
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
          ...
          <div><span className="text-muted-foreground">WhatsApp: </span><span className="font-medium">{lead.whatsapp || "-"}</span></div>
          <div><span className="text-muted-foreground">Service: </span><span className="font-medium">{lead.service || "-"}</span></div>
          <div><span className="text-muted-foreground">City: </span><span className="font-medium">{lead.mailing_city || "-"}</span></div>
          <div>
          ...
          <div><span className="text-muted-foreground">Date: </span><span className="font-medium">{new Date(lead.created_at).toLocaleDateString()}</span></div>
        </div>
        {lead.message && (
      ...
      <main className="container mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="mb-6 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">My Assigned Leads ({assignedLeads.length})</h2>
          </div>
