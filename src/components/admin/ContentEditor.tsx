  ...
  return <div className="space-y-6">
      ...
      return <Card key={content.id}>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {!isEditing ? <Button variant="outline" size="sm" onClick={() => startEdit(content)}>
            ...
            <CardContent className="space-y-4">
            ...
            })} className="mt-1" /> : <p className="text-sm text-muted-foreground mt-1">{content.title || "-"}</p>}
              </div>

              <div>
                <Label>Content</Label>
                {isEditing ? <Textarea value={editData.content || ""} onChange={e => setEditData({
              ...editData,
              content: e.target.value
            })} className="mt-1 min-h-[100px]" /> : <p className="text-sm text-muted-foreground mt-1 whitespace-pre-wrap">
                    {content.content || "-"}
                  </p>}
              </div>

              {/* Hero-specific fields */}
              {content.section_key === "hero" && <div>
                  <Label>Background Image URL</Label>
                  {isEditing ? <Input value={metadata?.backgroundImage || ""} onChange={e => updateMetadataField("backgroundImage", e.target.value)} className="mt-1" placeholder="https://..." /> : <p className="text-sm text-muted-foreground mt-1 truncate">
              ...
              {content.section_key === "offices" && <div>
                  <Label>Office Locations (JSON format)</Label>
                  {isEditing ? <div className="space-y-3 mt-2">
                      {((metadata?.offices || []) as Office[]).map((office, index) => <div key={index} className="p-3 border border-border rounded-lg space-y-2">
                    ...
                    updateMetadataField("offices", offices);
                          ...
                          <Input value={office.city} onChange={e => {
                  const offices = [...((metadata?.offices || []) as Office[])];
                  offices[index] = {
                    ...offices[index],
                    city: e.target.value
                  ...
                  updateMetadataField("offices", offices);
                }} placeholder="City name" className="text-sm" />
                          <Input value={office.address} onChange={e => {
                  const offices = [...((metadata?.offices || []) as Office[])];
                  offices[index] = {
                    ...offices[index],
                  ...
                  updateMetadataField("offices", offices);
                }} placeholder="Full address" className="text-sm" />
                          <Input value={office.whatsapp || ""} onChange={e => {
                  const offices = [...((metadata?.offices || []) as Office[])];
                  offices[index] = {
                    ...offices[index],
                  ...
                  updateMetadataField("offices", offices);
                }} placeholder="WhatsApp (e.g. 917410030418)" className="text-sm" />
                          <Input value={office.phone || ""} onChange={e => {
                  const offices = [...((metadata?.offices || []) as Office[])];
                  offices[index] = {
                    ...offices[index],
                ...
                updateMetadataField("offices", offices);
                      ...
                      {((content.metadata as any)?.offices || []).map((office: Office, i: number) => <div key={i} className="text-sm">
              ...
              {content.section_key === "contact_details" && <div>
                  <Label>Phone Numbers & Emails</Label>
                  {isEditing ? <div className="space-y-4 mt-2">
                      <div>
                        <Label className="text-xs">Phone Numbers</Label>
                        {((metadata?.phones || []) as string[]).map((phone, index) => <div key={index} className="flex gap-2 mt-1">
                            <Input value={phone} onChange={e => {
                    const phones = [...((metadata?.phones || []) as string[])];
                    phones[index] = e.target.value;
                    updateMetadataField("phones", phones);
                  }} placeholder="+91-XXX-XXXX-XXX" className="text-sm" />
                            <Button type="button" variant="ghost" size="sm" onClick={() => {
                    const phones = [...((metadata?.phones || []) as string[])];
                  ...
                  updateMetadataField("phones", phones);
                      ...
                      <div>
                        ...
                        {((metadata?.emails || []) as string[]).map((email, index) => <div key={index} className="flex gap-2 mt-1">
                  ...
                  updateMetadataField("emails", emails);
                    ...
                    </div> : <div className="space-y-1 mt-2 text-sm text-muted-foreground">
                      {((content.metadata as any)?.phones || []).map((p: string, i: number) => <div key={`p${i}`}>📞 {p}</div>)}
                      {((content.metadata as any)?.emails || []).map((e: string, i: number) => <div key={`e${i}`}>✉️ {e}</div>)}
                    </div>}
                </div>}

              {content.section_key === "settings" && <div>
                  <Label>WhatsApp Number (without + symbol, e.g., 917410030418)</Label>
                  {isEditing ? <Input value={metadata?.whatsapp_number || ""} onChange={e => updateMetadataField("whatsapp_number", e.target.value)} className="mt-1" placeholder="917410030418" /> : <p className="text-sm text-muted-foreground mt-1">
