...
import {
  Loader2, User, Phone, MapPin, Send, Calendar, Navigation, ScanLine,
  LandPlot, MessageSquare, Home, Building2, Mail as MailIcon, Wrench,
  LocateFixed, Ruler, Copy, ExternalLink, IndianRupee, ShieldCheck
} from "lucide-react";
import {
...
const SERVICES_LIST = ["GWSR", "WHi", "GEOS", "GeoTech", "THRML-IMG", "GPRS", "TPE", "TPCG", "WANO", "CBME", "P"];
...
const TO_SQFT: Record<AreaUnit, number> = {
  sqft: 1, sqm: 10.7639, sqyd: 9, acres: 43560, guntas: 1089
};
...
const EnquiryForm = ({ serviceTitle, onSuccess }: EnquiryFormProps) => {
  ...
  // Load reCAPTCHA site key from admin settings
  useEffect(() => {
    const loadKey = async () => {
      try {
        ...
        if (data?.value && typeof data.value === "string") {
          setRecaptchaSiteKey(data.value);
        } else if (data?.value && typeof data.value === "object" && (data.value as any).key) {
          setRecaptchaSiteKey((data.value as any).key);
        }
  ...
  const autoDescription = useMemo(() => {
    ...
    if (totalBizCost) parts.push(`Total BIZ COST: ₹${totalBizCost}`);
    if (coords) {
    ...
    return parts.join("\n");
  }, [lastName, whatsapp, serviceNeeded, areaType, areaValue, areaUnit, converted, bizArea, distance, numScans, totalBizCost, coords, mailingStreet, mailingCity, mailingPoBox, detectedCountry]);
      ...
      async (pos) => {
        ...
        try {
          ...
          if (data.address) {
            ...
            if (code && !whatsapp) setWhatsapp(code + " ");
          }
  ...
  return (
    <form
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
        <Label className={labelCls}><User className="h-3.5 w-3.5 text-primary" /> Last Name <span className="text-destructive">*</span></Label>
        <div className="relative">
          <User className={fieldIcon} />
          <Input name="lastname" required maxLength={100} value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Your last name" className={inputCls} />
        </div>
      </motion.div>
      ...
      {/* WhatsApp */}
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
        <Label className={labelCls}><Phone className="h-3.5 w-3.5 text-primary" /> WhatsApp Num <span className="text-destructive">*</span></Label>
        <div className="relative">
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible">
        <Button
          type="button" variant="outline" onClick={handleGetLocation} disabled={locating}
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          <Label className={labelCls}><Navigation className="h-3 w-3 text-primary" /> Distance in KM <span className="text-destructive">*</span></Label>
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label className={labelCls}><Wrench className="h-3 w-3 text-primary" /> Service Needed <span className="text-destructive">*</span></Label>
        ...
        <div className="space-y-1.5">
          <Label className={labelCls}><ScanLine className="h-3 w-3 text-primary" /> Number of Scans <span className="text-destructive">*</span></Label>
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
        ...
        <div className="flex gap-2">
          ...
          <Input
            type="number" min="0" step="any" required value={areaValue}
            onChange={(e) => setAreaValue(e.target.value)} placeholder="Enter area"
      ...
      {/* Total BIZ COST */}
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
        <Label className={labelCls}><IndianRupee className="h-3.5 w-3.5 text-primary" /> Total BIZ COST <span className="text-destructive">*</span></Label>
        <div className="relative">
          <IndianRupee className={fieldIcon} />
          <Input
            name="cf_1020" type="number" min="0" step="any" required
            value={totalBizCost} onChange={(e) => setTotalBizCost(e.target.value)}
            placeholder="Enter estimated cost" className={inputCls} />
        </div>
      </motion.div>
      ...
      <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Auto-filled from above details..." rows={4}
      ...
      {recaptchaSiteKey && (
        <motion.div custom={idx++} variants={stagger} initial="hidden" animate="visible" className="space-y-1.5">
          <Label className={labelCls}><ShieldCheck className="h-3.5 w-3.5 text-primary" /> Security Verification</Label>
          <div ref={recaptchaRef} className="flex justify-center" />
          {!recaptchaToken && (
            <p className="text-[10px] text-muted-foreground italic text-center">Please verify you are not a robot</p>
          )}
        </motion.div>
      )}
